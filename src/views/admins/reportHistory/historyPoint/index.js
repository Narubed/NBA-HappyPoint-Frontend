...
function index() {
  ...
  useEffect(async () => {
    const report = await axios.get(`${process.env.REACT_APP_HAPPY_POINT_BACKEND}/point_history`);
    const members = await axios.get(`${process.env.REACT_APP_HAPPY_POINT_BACKEND}/members`);

    const newReport = [];
    if (report && members) {
      report.data.data.forEach((element) => {
        const findMemberId = members.data.data.find((item) => item._id === element.ph_member_id);
        if (findMemberId) {
          newReport.push({
            ...element,
  ...
  const filterDate = (newValue) => {
    const filterReport = isReport.filter(
      (item) =>
        dayjs(item.ph_timestamp).format('DD MMMM YYYY') === dayjs(newValue).format('DD MMMM YYYY')
    );
