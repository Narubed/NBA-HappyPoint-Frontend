...
export default function CreateMembers() {
  ...
  const [values, setValues] = React.useState({
    usep_name: valueState.usep_name,
    usep_point: valueState.usep_point,
    usep_grop_level: valueState.usep_grop_level,
    usep_limited_total: valueState.usep_limited_total,
    usep_limited_member: valueState.usep_limited_member,
    usep_date_start: valueState.usep_date_start,
    usep_date_end: valueState.usep_date_end,
    usep_detail: valueState.usep_detail,
    usep_status: valueState.usep_status,
    usep_image: valueState.usep_image,
    usep_note: valueState.usep_note
  });
  const filterLevel = [];
  if (valueState.usep_grop_level) {
    // eslint-disable-next-line array-callback-return
    valueState.usep_grop_level.map((element) => {
      const idx = valueState.levels.find((index) => index._id === element);
      filterLevel.push(idx);
    });
  }

  const [levels, setLevels] = React.useState(valueState.levels);
    ...
    if (
      ...
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          await axios
            .put(`${process.env.REACT_APP_HAPPY_POINT_BACKEND}/use_point/${valueState._id}`, data)
            .then(() => {
              Swal.fire({
                icon: 'success',
  ...
  const [editorState, seteditorState] = useState(
    EditorState.createWithContent(
      ContentState.createFromBlockArray(convertFromHTML(valueState.usep_detail))
    )
  );
